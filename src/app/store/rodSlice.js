...
import { StationValueData } from "../(stations)/(RO)/rod/(data)/rodData";
...
export const runCalculationNow = createAsyncThunk(
  "rod/runCalculationNow",
  async (_, { getState, dispatch }) => {
    const { stationData } = getState().rod;
    if (!stationData) return;

    const newData = stationData.map((row) =>
      row.map((cell) => {
        // ✅ استثناء الحقول J و N
        if (cell.key === "Ja" || cell.key === "Jc" || cell.key === "Jd" || cell.key === "Na" | cell.key === "Nc") {
          return cell;
        }
...
export const fetchSavedFiles = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await fetch("/api/rodData");
...
export const saveProject = createAsyncThunk(
  ...
  async ({ fileName, stationData }, { getState, dispatch, rejectWithValue }) => {
    try {
      ...
      const { selectedFile } = getState().rod
...
export const rodSlice = createSlice({
  name: "rod",
  initialState,
  reducers: {
...
export default rodSlice.reducer;