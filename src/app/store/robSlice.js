...
import { StationValueData } from "../(stations)/(RO)/rob/(data)/robData";
...
export const runCalculationNow = createAsyncThunk(
  "rob/runCalculationNow",
  async (_, { getState, dispatch }) => {
    const { stationData } = getState().rob;
    if (!stationData) return;

    const newData = stationData.map((row) =>
      row.map((cell) => {
        // ✅ استثناء الحقول J و N
        if (cell.key === "Ja" || cell.key === "Jb" || cell.key === "N") {
          return cell;
        }
...
export const fetchSavedFiles = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await fetch("/api/robData");
...
export const saveProject = createAsyncThunk(
  ...
  async ({ fileName, stationData }, { getState, dispatch, rejectWithValue }) => {
    try {
      ...
      const { selectedFile } = getState().rob
...
export const robSlice = createSlice({
  name: "rob",
  initialState,
  reducers: {
...
export default robSlice.reducer;