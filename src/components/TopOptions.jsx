...
const TopOptions = ({ station = "MSF" }) => {
  ...
  const [selectedRun, setSelectedRun] = useState(runData[0]);

  // تعطيل العناصر الأخرى إذا لم يتم اختيار ملف صالح
  const isDisabled = !selectedFile || selectedFile === "New Plant";
  ...
  return (
    <div className="bg-white p-4 flex flex-col sm:flex-row sm:items-center sm:justify-around gap-4 sm:gap-6 w-full">
      ...
      {/* Save to Dashboard / External Link */}
      <div className="flex flex-col w-full sm:w-auto">
        ...
        <div className="flex w-full sm:w-auto hover:underline">
          <button className="flex items-center gap-3 text-blue-600 w-full sm:w-auto cursor-pointer disabled:opacity-50"
           disabled={isDisabled}>
